...
due to precession can be approximated by equations (1.2) and (1.3) in the text @bob.

$$
\Delta\alpha = M + N \sin\alpha \tan\alpha
$$ {#eq-change-right-ascension}

$$
\Delta\delta = N \cos\alpha
$$ {#eq-change-declination}

## Target Motion {#sec-target-motion}

The motion of objects in space too causes the declination and right ascension angles
to _drift_. With a star's radial velocity with respect to an observer defined as $v_r$, and
the star's **transverse velocity** defined as $v_\theta$, the star's proper motion $\mu$ 
is given by equation (1.5) in the text @bob. 
